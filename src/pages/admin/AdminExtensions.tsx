...
const empty: Omit<Item, "id"> = { title: "", description: "", image_url: "", activity_date: "" };
...
const AdminExtensions = () => {
  ...
  const save = async () => {
    ...
    setLoading(true);
    const payload = { title: editing.title, description: editing.description, image_url: editing.image_url, activity_date: editing.activity_date } as any;
    if (editing.id) {
      await supabase.from("extensions" as any).update(payload).eq("id", editing.id);
    } else {
      await supabase.from("extensions" as any).insert(payload);
    }
    setEditing(null);
    setLoading(false);
    fetchData();
  ...
  const remove = async (id: string) => {
    if (!confirm("Delete this extension activity?")) return;
    await supabase.from("extensions" as any).delete().eq("id", id);
    fetchData();
  };

  const uploadImage = async (file: File) => {
    const path = `extensions/${Date.now()}_${file.name}`;
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-foreground">Extension Activities</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/50 backdrop-blur-sm p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-lg rounded-2xl bg-card p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-lg font-semibold text-card-foreground">{editing.id ? "Edit" : "Add"} Extension</h2>
            ...
            <div className="space-y-3">
              ...
              <textarea placeholder="Description" rows={4} value={editing.description || ""} onChange={e => setEditing({ ...editing, description: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" />
              <input placeholder="Activity Date (e.g. March 15, 2026)" value={editing.activity_date || ""} onChange={e => setEditing({ ...editing, activity_date: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" />
              <div>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                ...
                <td className="px-4 py-3 text-muted-foreground">{item.activity_date}</td>
            ...
            {items.length === 0 && <tr><td colSpan={3} className="px-4 py-8 text-center text-muted-foreground">No extension activities yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
